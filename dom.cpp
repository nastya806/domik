...
std::shared_ptr<SDL_Texture> Make_icon(SDL_Renderer *renderer, const char *name)
{
	...
	if(picture == nullptr)
	{
		std::cerr << "Картинка не загружается" <<
...
int main(int, char **)
{
	...
	bool f = true;
	int icon_x = 800, icon_y = 10;// координаты облака
	bool f2 = true;
	int icon_x2 = 500, icon_y2 = 550;// координаты винни
 ...
 //Забор
        for (int i = 20; i < WIDTH; i += 100) {
        	SDL_SetRenderDrawColor(ren.get(), 245, 15, 199, 1);// палки на заборе
            SDL_Rect r1 { i, 450, 50, 100 };
            SDL_RenderFillRect(ren.get(), &r1);
            SDL_Rect r2 { i + 50, 490, 50, 20 };
            SDL_RenderFillRect(ren.get(), &r2);
            SDL_SetRenderDrawColor(ren.get(), 245, 15, 65, 1);// треугольник на заборе
  ...
  //Окно
        ...
        SDL_SetRenderDrawColor(ren.get(), 242, 255, 0, 1);//внутри
        SDL_RenderFillRect(ren.get(), &r2);

        SDL_SetRenderDrawColor(ren.get(), 107, 75, 26, 1);// палочки
